...
const CurrentMonthTotalContentsWrapper = () => {
  ...
  useEffect(() => {
    if (energyMonthlyData) {
      setSeries([
        ...
        {
          name: t('Current Month Bill Status'),
          data: getFilteredSeriesValuesExtends({
            type: 'date',
            element: energyMonthlyData.thisMonthData,
            monthValue: moment(currentDate).format('YYYY-MM'),
        ...
        {
          name: t('AI Predicted Bill Status'),
          data: getFilteredSeriesValuesExtends({
            type: 'date',
            element: energyMonthlyData.thisMonthData,
            monthValue: moment(currentDate).format('YYYY-MM'),
  ...
  useEffect(() => {
    ...
    if (
      ...
      series[2].data.length === xAxisDataLength &&
      series[3].data.length === xAxisDataLength
    ) {
  ...
  return (
    <>
      {seriesIsLoading ? (
        ...
        <DoubleLineChart
          height={350}
          colors={['#0EA5FF', '#8ED9CA', '#3C50E0', '#ff0000']}
          series={series}
        />
      )}
    </>
...
export default CurrentMonthTotalContentsWrapper;
