...
interface PastEnergyUsageHistoryContentsWrapperProps {
  selectedDates: string[];
  setSelectedDates: (dates: string[]) => void;
  setSearchedData: (searchedData: any[]) => void;
}

const PastEnergyUsageHistoryContentsWrapper: React.FC<
  PastEnergyUsageHistoryContentsWrapperProps
> = ({ selectedDates, setSelectedDates, setSearchedData }) => {
  ...
  const handleDownload = async (dates: string[]) => {
    if (loggedInUser?.role === E_ROLE_TYPES.USER) {
      ...
      try {
        // energy/usage -> /report/energy/usage/{companyId}
        // fetchData 함수 호출하여 Blob 데이터 얻기
        const blob = await fetchExcelData(
          `/report/energy/usage/${loggedInUser?.companyId}?startDate=${dates[0]}&endDate=${dates[1]}`,
        );
        ...
        a.href = url;
        a.download = `Energy_Usage_Report ${dates[0]} ~ ${dates[1]}.xlsx`; // 파일 이름 설정
  ...
  // 날짜 범위에 따라 차트를 업데이트
  useEffect(() => {
    // 검색된 PastEnergyUsageHistory 데이타
...
export default PastEnergyUsageHistoryContentsWrapper;
