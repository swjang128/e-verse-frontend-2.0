...
interface PastBillStatusHistoryContentsWrapperProps {
  selectedDates: string[];
  setSelectedDates: (dates: string[]) => void;
  setSearchedData: (searchedData: any[]) => void;
}

const PastBillStatusHistoryContentsWrapper: React.FC<
  PastBillStatusHistoryContentsWrapperProps
> = ({ selectedDates, setSelectedDates, setSearchedData }) => {
  ...
  const handleDownload = async (dates: string[]) => {
    if (loggedInUser?.role === E_ROLE_TYPES.USER) {
      ...
      try {
        // energy/billing-overview -> /report/energy/bill/{companyId}
        // fetchData 함수 호출하여 Blob 데이터 얻기
        const blob = await fetchExcelData(
          `/report/energy/bill/${loggedInUser?.companyId}?startDate=${dates[0]}&endDate=${dates[1]}`,
        );
        ...
        a.href = url;
        a.download = `Energy_Bill_Report ${dates[0]} ~ ${dates[1]}.xlsx`; // 파일 이름 설정
...
export default PastBillStatusHistoryContentsWrapper;
